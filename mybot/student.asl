...
sq(1, " Soy Jorge y quiero darme de alta en SI ").
sq(2, " Buenas me llamo Jorge y quiero cambiarme al grupo reducido 2 de SI").
sq(3, " Buenas me llamo Juan y quiero reservar un seminario ").
sq(4, " Soy Jorge y quiero solicitar las practicas en Mercadona"). 
sq(5, " Soy gabriel y quiero solicitar la asignacion del tema desarollo web123").
sq(6, " Hola me llamo gabriel y quiero solicitar la asignacion del tema sistema inteligente en android").
sq(7, " Soy gabriel y quiero solicitar la defensa del tema 4412D Desarrollo Web").
sq(8, " Soy Juan y quiero solicitar la evaluacion por compensacion de SI").
sq(9, " Me llamo Ramon y quiero anadir Operaciones Cuanticas a tfg").
sq(10, " Hola me llamo pedro y recuerdame que vaya pronto a la cama").
...
/* Initial goals */
	...
	//selecttq acontrola que no se escojan preguntas repetidas de forma tq
	
	+!selecttq(N,Answer) : dotq(N,_) <- ?tq(New,Answer); -tq(New,Answer); +dotq(New,Answer).
	+!selecttq(N,Answer) : not dotq(N,_) & tq(N,Answer) <- -tq(N,Answer); +dotq(N,Answer).
	+!selecttq(0,Answer) : tq(N,Answer) <- -tq(N,Answer); +dotq(N,Answer).
	
	//selectntq controla que no se escojan preguntas repetidas de forma ntq
	+!selectntq(N,Answer) : dontq(N,_) <- ?ntq(New,Answer); -ntq(New,Answer); +dontq(New,Answer).
	+!selectntq(N,Answer) : not dontq(N,_) & ntq(N,Answer) <- -ntq(N,Answer); +dontq(N,Answer).
	+!selectntq(0,Answer) : ntq(N,Answer) <- -ntq(N,Answer); +dontq(N,Answer).
	
	//selectaq controla que no se escojan preguntas repetidas de forma aq
	+!selectaq(N,Answer) : doaq(N,_) <- ?aq(New,Answer); -aq(New,Answer); +doaq(New,Answer).
	+!selectaq(N,Answer) : not doaq(N,_) & aq(N,Answer) <- -aq(N,Answer); +doaq(N,Answer).
	+!selectaq(0,Answer) : aq(N,Answer) <- -aq(N,Answer); +doaq(N,Answer).
	
	//selectsq controla que no se escojan preguntas repetidas de forma sq
	+!selectsq(N,Answer) : dosq(N,_) <- ?sq(New,Answer); -sq(New,Answer); +dosq(New,Answer).
	+!selectsq(N,Answer) : not dosq(N,_) & sq(N,Answer) <- -sq(N,Answer); +dosq(N,Answer).
	+!selectsq(0,Answer) : sq(N,Answer) <- -sq(N,Answer); +dosq(N,Answer).
	
		//mantener una conversacion segun preguntas tq
		+!preguntartq <-
			.println;
			.println("%%%%%%%%%%%%%%%%%% PREGUNTAS DE LA FORMA TQ %%%%%%%%%%%%%%%%%%");
			for(.range(I,1,5)){
				.println("=======================================================================");
				N = math.round(math.random(17))+1;
				!selecttq(N, Pregunta);
				.println("Pregunta formulada: ", Pregunta);
				chat(Pregunta);
				.wait(1500);
			}.
		
		//mantener una conversacion segun preguntas ntq
		+!preguntarntq <-
			.println;
			.println("%%%%%%%%%%%%%%%%%% PREGUNTAS DE LA FORMA NTQ %%%%%%%%%%%%%%%%%%");
			for(.range(I,1,5)){
				.println("=======================================================================");
				
				N =math.round(math.random(53))+1;
				!selectntq(N, Pregunta);
				.println("Pregunta formulada: ", Pregunta);
				chat(Pregunta);
				.wait(1500);
				chat("Si");
		...
		//mantener una conversacion segun preguntas aq
		+!preguntaraq <-
			.println;
			.println("%%%%%%%%%%%%%%%%%% PREGUNTAS DE LA FORMA AQ %%%%%%%%%%%%%%%%%%");
			for(.range(I,1,5)){
				.println("=======================================================================");
		...
		//preguntar por alguno de los servicios del ANEXO II (Servicios)
		+!preguntarsq <-
			.println;
			.println("%%%%%%%%%%%%%%%%%% SOLICITUDES DE REALIZACION DE SERVICIOS DEL ANEXO II (SERVICIOS)%%%%%%%%%%%%%%%%%%");
			for(.range(I,1,10)){
				.println("=======================================================================");
	...
	//filtrar las contestaciones del bot que no incluyan servicios en nombre de
	+answer(Answer) : not service(Answer) <-
	?respuesta(N);
	-+respuesta(N+1);
	+respuesta(N,Answer);
	.println("===============================================");
